...
[최신 릴리스](https://github.com/autobile/autobile/releases/latest)에서 `autobile-0.2.0.apk`를
...
1~3단계는 모두 *이름이 있는* 요소를 찾는 방식입니다 — id, 라벨, 설명. 그런 게 아예 없는 앱이
있습니다. 게임은 화면을 직접 그리고, 동영상 플레이어나 Unity·Unreal로 만든 앱도 마찬가지입니다.
안드로이드 접근성 계층에서 보면 화면 전체가 빈 사각형 하나입니다.

이런 앱에서는 Autobile이 화면을 보고 *어디를* 눌러야 할지 판단한 뒤 그 자리를 누릅니다. 가장
