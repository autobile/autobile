...
[최신 릴리스](https://github.com/autobile/autobile/releases/latest)에서 APK를
...
1~3단계는 모두 *이름이 있는* 요소를 찾는 방식입니다 — id, 라벨, 설명. 늘 가능한 건 아닙니다.
게임은 화면을 직접 그리고, 동영상 플레이어나 Unity·Unreal로 만든 앱도 마찬가지입니다. 접근성
계층에서 보면 화면 전체가 빈 사각형 하나입니다. 더 까다로운 경우도 있습니다 — 요소는 가득한데
그중 사람이 알아볼 만한 이름을 가진 게 하나도 없는 화면입니다.

두 경우 모두, 접근성 트리가 답을 주지 못하면 Autobile이 화면을 보고 *어디를* 눌러야 할지 판단한
뒤 그 자리를 누릅니다. 가장
마지막에 시도하며 절대 먼저 쓰지 않습니다. 위치를 겨냥하는 건 이 앱이 피하려는 취약함 그
자체니까요. 다만 그런 앱을 아예 다루지 못하는 쪽이 더 나쁜 답입니다. 스크린샷 전송은 아래에
설명한 별도 스위치라, 이 경로는 켜두신 경우에만 존재합니다.
...
1. **설정 → 클라우드 보조 → 클라우드 사용** 열기
2. 공급자 선택
3. 해당 공급자의 API 키를 붙여넣거나, 실험적인 ChatGPT 구독 공급자에서 브라우저 로그인

엔드포인트는 기본적으로 Google Gemini API를 쓰지만 수정할 수 있으므로, 직접 운영하는 호환
서버를 지정해도 됩니다. 구독 로그인은 Codex의 PKCE 브라우저 흐름을 따르고 만료 전에 권한을
갱신합니다. OpenAI 공식 문서는 Codex 클라이언트의 ChatGPT 로그인을 설명하지만 제3자 연동을
안정적인 공개 API로 보장하지는 않습니다. 구독 표면이 바뀔 경우 API 키 공급자를 사용하세요.
...
자동 단위 테스트와 실기기에서 축소된 릴리스 빌드로 실행되는 계측 테스트가 이를 검증합니다.
